...
def test_clustering_pipeline_deterministic():
    ...
    model, scaler, scored_df, result = fit_and_persist(
        df,
...
def test_reproducibility():
    ...
    model1, _, scored1, _ = fit_and_persist(df, random_state=42)
    model2, _, scored2, _ = fit_and_persist(df, random_state=42)
...
def test_label_clusters_maps_risk_flags():
    df = pd.DataFrame({
        ...
        "freq_trend_ratio": [0.5] * 25 + [1.5] * 25,
